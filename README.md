...
This colors by quantiles so any artibutrary number is allowed. However, the colormapping has been cut off from [0, 1] to [.3, 1] since 0 (i.e., white) comes out looking too faint. Thus, for quantiles larger than about 5 or 6, differences become hard to see. Here is one with `strength=25`.
...
![Wifi plot with others]()

```
fig, axes = plot_most_macs(macdf = freq_w, m='.', plot_others=True)
axes.grid(False)
axes.set_title('Top WiFi Networks')
axes.set_ylabel('Unique WiFi Networks')
axes.set_ylim([-8, 101])
...
![Wifi plot with strength and others]()
